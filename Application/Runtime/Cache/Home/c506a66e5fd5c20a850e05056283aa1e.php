...
<?php else: ?>
  ...
  <?php if(!isAdmin()) exit('Error'); ?>
<title>管理 - <?php echo C('SITE_TITLE');?></title>
<script type="text/javascript">
function node_set(id,allow){
    $.ajax({
      type:"POST",
      url:"<?php echo U('/Home/Admin/register_node');?>",
      data:{
          nid:id,
          allow:allow
        },
...
<div class="col-md-8">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <?php if(is_array($talkclass)): $i = 0; $__LIST__ = $talkclass;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="col-md-4" id="node_<?php echo ($vo['id']); ?>">
        <div class="panel panel-default">
          <div class="panel-body">
          <div class="panel-body">
            <p><strong>节点名：</strong><?php echo $vo['name'];?><br/>
            <strong>父节点：</strong><?php echo getTalkclassname($vo['father']);?><br/>
            <strong>申请人：</strong><?php echo getUsername($vo['uid']);?></p>
            <div class="text-right">
              <button class="btn btn-success" onclick="node_set(<?php echo ($vo['id']); ?>,1)">通过</button>
              <button class="btn btn-danger" onclick="node_set(<?php echo ($vo['id']); ?>,0)">拒绝</button>
            </div>
          </div>
        </div>
        </div>
